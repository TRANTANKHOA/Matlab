...
%             saveas(handles.figlist(ind(i)),fullfile(pn,filetype{j},fn),filetype{j})
            export_fig([fullfile(pn,filetype{j},fn) '.' filetype{j}],handles.figlist(ind(i)),'-a1',['-r',num2str(300)]);
        else
%             saveas(handles.figlist(ind(i)),fullfile(pn,fn),filetype{j})
            export_fig([fullfile(pn,fn) '.' filetype{j}],handles.figlist(ind(i)),'-a1',['-r',num2str(300)]);
        end
    end
end
nfigsave = n;
